...
package net.thijnmens.enhanchedtools.world;

import org.jline.terminal.Size;

import net.thijnmens.enhanchedtools.item.ItemTheVoid;
import net.thijnmens.enhanchedtools.block.BlockReinforcedObsidian;
...
public class WorldTheVoid extends ElementsEnhanchedTools.ModElement {
	public static int DIMID = 3;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("enhanchedtools:thevoid_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldTheVoid(ElementsEnhanchedTools instance) {
		super(instance, 11);
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemTheVoid().setUnlocalizedName("thevoid").setRegistryName("thevoid"));
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemTheVoid.block, 0, new ModelResourceLocation("enhanchedtools:thevoid", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag
											? BlockReinforcedObsidian.block.getDefaultState().getBlock().getDefaultState()
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? BlockReinforcedObsidian.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockReinforcedObsidian.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockReinforcedObsidian.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockReinforcedObsidian.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockReinforcedObsidian.block
